...
public class HighLowDayIT extends Configured implements Tool {
    ...
    public static void main(String[] args) throws Exception {
        HighLowDayIT driver = new HighLowDayIT();
    ...
    public void tearDown() {
        try {
            if (qdbInstance != null) {
                // Check results
                Text result = qdbInstance.get("1986-10-06");
                assertTrue("High:29.4 Low:29.4".equals(result.toString().trim()));
                result = qdbInstance.get("2010-02-08");
                assertTrue("High:23.18 Low:0.79".equals(result.toString().trim()));
